% Looking for maxima in retrograde spanwise vorticity AND dudy opposite sign to the mean AND v outflow
...
%mprofile=matfile('../data/mean_profiles.mat');
...
for time=1:1%tstart:tstep:tend
	...
	vj=vj.*(vfieldb(:,:,jcond)<0);
	vjt=vjt.*(vfieldt(:,:,jcond)>0);
%	vj=vj.*(vfieldb(:,:,jcond)>vthreshold);
%       vjt=vjt.*(vfieldt(:,:,jcond)<vthreshold);
    clear m
...
fc=sprintf("../data/conditionaln_jcond_omegaz_%03d.mat",jcond);
