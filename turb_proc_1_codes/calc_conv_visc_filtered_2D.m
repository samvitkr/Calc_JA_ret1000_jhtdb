...
%	omegay(j,:,:)=ifft2(fft2(squeeze(omegay(j,:,:))).*fbp(:,:,j)	,'symmetric');
	...
	v(j,:,:)=ifft2(         fft2(squeeze( v(j,:,:))).*f(:,:,j)    ,'symmetric');
        w(j,:,:)=ifft2(         fft2(squeeze( w(j,:,:))).*f(:,:,j)    ,'symmetric');
        omegaz(j,:,:)=ifft2(fft2(squeeze(omegaz(j,:,:))).*f(:,:,j)    ,'symmetric');
        omegay(j,:,:)=ifft2(fft2(squeeze(omegay(j,:,:))).*f(:,:,j)    ,'symmetric');
        omegax(j,:,:)=ifft2(fft2(squeeze(omegax(j,:,:))).*f(:,:,j)    ,'symmetric');	
...
%	total=viscous_x+convective_x;
	...
	fl=sprintf("Transfer_f_2Dinertial_%03d",time)
