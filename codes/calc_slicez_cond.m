...
%	fx=sprintf('../data/lse_xsliceset_j_%03d.mat',jcond)
	...
	ud=  squeeze(m.u(kslice,:,:))';	
	vd=  squeeze(m.v(kslice,:,:))';
	wd=  squeeze(m.w(kslice,:,:))';
	...
	vozd=squeeze(m.v(kslice,:,:).*(m.dvdx(kslice,:,:)-m.dudy(kslice,:,:)))';
        ...
        woydc=squeeze(m.woy(kslice,:,:))';

%u2d=  squeeze(m.u2(kslice,:,:))';
%v2d=  squeeze(m.v2(kslice,:,:))';
%w2d=  squeeze(m.w2(kslice,:,:))';
%ox2d=  squeeze(m.ox2(kslice,:,:))';
