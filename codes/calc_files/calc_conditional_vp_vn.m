...
for time=tstart:tstep:tend
    ...
    for k = 1:num_layers
        ...
        vb = squeeze(vfieldb(:,:,jcond)); vt =squeeze( vfieldt(:,:,jcond));
      ...
      vj_q2 = vb.*(vb > vthreshold(k));
      vj_q4 = vb.*(vb < -vthreshold(k));
      vjt_q2= vt.*(vt > vthreshold(k));
      vjt_q4= vt.*(vt < -vthreshold(k));
        for eType = 1:2
...
for k = 1:num_layers
    jcond = configs(k, 1);
    mc2 = matfile(sprintf("../../data/conditional_vpos_jcond_%03d.mat", jcond), 'Writable', true);
    ...
    mc4 = matfile(sprintf("../../data/conditional_vneg_jcond_%03d.mat", jcond), 'Writable', true);
