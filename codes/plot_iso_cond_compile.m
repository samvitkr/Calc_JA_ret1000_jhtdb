...
title( labels(1+3*(j-1)) )
subplot(5,5,[3+5*(j-1) 4+5*(j-1)])
m=m2;
...
ylabel(c,"$(v\omega_z - w\omega_y)/(-u_{\tau}^2/H)$",'interpreter','latex','FontSize',10)
c.Ticks=[-cl, 0, cl];
view(45,45)
clim([-cl cl])
%print(h1,'isotryflp','-dpng');
...
%set(gca,'Position',[0.75 0.15 0.2 0.8])
 ...
 saveas(h1,'eddyflux_compiled.fig')
