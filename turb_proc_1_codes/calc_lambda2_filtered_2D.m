...
%	Gz(j,:)= exp(-( yl(j).^2*(kz.^2)./Deltaz^2));
	...
	S_11(j,:,:)=ifft2(  fft2(squeeze( S_11(j,:,:))).*fbp(:,:,j),'symmetric');
	S_12(j,:,:)=ifft2(  fft2(squeeze( S_12(j,:,:))).*fbp(:,:,j),'symmetric');
	S_13(j,:,:)=ifft2(  fft2(squeeze( S_13(j,:,:))).*fbp(:,:,j),'symmetric');
	S_22(j,:,:)=ifft2(  fft2(squeeze( S_22(j,:,:))).*fbp(:,:,j),'symmetric');
	S_23(j,:,:)=ifft2(  fft2(squeeze( S_23(j,:,:))).*fbp(:,:,j),'symmetric');
	S_33(j,:,:)=ifft2(  fft2(squeeze( S_33(j,:,:))).*fbp(:,:,j),'symmetric');

	O_21(j,:,:)=ifft2(  fft2(squeeze( O_21(j,:,:))).*fbp(:,:,j),'symmetric');
	O_13(j,:,:)=ifft2(  fft2(squeeze( O_13(j,:,:))).*fbp(:,:,j),'symmetric');
	O_32(j,:,:)=ifft2(  fft2(squeeze( O_32(j,:,:))).*fbp(:,:,j),'symmetric');
	...
	fl=sprintf("lambda_fhp_2Dinertial_%03d",time)
