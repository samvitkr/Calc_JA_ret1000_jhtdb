...
fnprev=sprintf("velfield_%02d_%03d.mat",proc-1,time);
...
mnprev=matfile(fnprev);

%mn.ufield(:,:,1:64)=mnprev.ufield(:,:,193:256);
%mn.vfield(:,:,1:64)=mnprev.vfield(:,:,193:256);
%mn.wfield(:,:,1:64)=mnprev.wfield(:,:,193:256);

	for k =(127):(64+128)% (Nz/nproc)
	    k
...
%	toc
	%toc
		...
		mn.ufield(:,:,k)=single(ufieldslice(:,:));
		mn.vfield(:,:,k)=single(vfieldslice(:,:));
		mn.wfield(:,:,k)=single(wfieldslice(:,:));
		toc
	end
%fnprev=sprintf("velfield_%02d_%03d.mat",proc-1,time);
%fn=sprintf("velfield_%02d_%03d.mat",proc,time);
%mn=matfile(fn,'Writable',true);
%mnprev=matfile(fnprev,'Writable',true);
%ufield(:,:,1:64)=mnprev.ufield(:,:,193:256);
%vfield(:,:,1:64)=mnprev.vfield(:,:,193:256);
%wfield(:,:,1:64)=mnprev.wfield(:,:,193:256);
%	mn.ufield=single(ufield);
%	mn.vfield=single(vfield);
%	mn.wfield=single(wfield);
toc
