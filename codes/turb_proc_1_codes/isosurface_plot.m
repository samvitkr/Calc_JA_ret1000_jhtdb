...
%fl=sprintf("lambda_f_2Dinertial_%03d.mat",time);
...
%mdata=matfile('isosurface_data_vrms.mat','Writable',true);
mdata=matfile('isosurface_data_lrmsf2D_oz.mat','Writable',true)
%load('JHTDB_RET1000.mat');
%v=mvel.v(nystart:nyend,1:nx,1:nz);
%vrms=sqrt(JHTDB_RET1000(nystart:nyend,5));
...
%voz = mcv.voz(:,1:nx,1:nz);
%woy = mcv.woy(:,1:nx,1:nz);
omegaz=mcv.omegaz(:,1:nx,1:nz);
omegay=mcv.omegay(:,1:nx,1:nz);
omegax=mcv.omegx(:,1:nx,1:nz);
